# Load all the color constants for colored writing
source "$INCLUDE_PATH/constants/colors.sh"

# Load all the logging constants
source "$INCLUDE_PATH/constants/logging.sh"

# Load all the file system constants
source "$INCLUDE_PATH/constants/filesystem.sh"

# Load all the SQLite constants
source "$INCLUDE_PATH/constants/sql.sh"

# Load the version constant
source "$INCLUDE_PATH/constants/version.sh"

# Load all the package manager constants
source "$INCLUDE_PATH/constants/package.sh"

# Load all the package manager constants
source "$INCLUDE_PATH/constants/migration.sh"
